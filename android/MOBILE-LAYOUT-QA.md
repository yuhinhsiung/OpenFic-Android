...
| 7 | 手机横屏时写作页三栏挤在一起，且都收不起来 | 写作页按 shell 的宽度断点判断是否渲染三栏，而横屏手机的宽度越过了断点；三栏的侧栏又都是 `collapsible={false}` | 手机 |
...
**现象**：手机横屏时，写作页的章节栏、编辑器、对话面板同时出现，每一栏都看不到多少东西，
而且都收不起来。

**根因**：写作页用 shell 的 `isMobile`（`window.innerWidth < 768`）决定渲染三栏还是单栏。
手机横屏视口约 915×412，宽度越过了断点 → 判为桌面 → 渲染三栏。而三栏的侧栏最小宽度
加起来就要 250 + 300 = 550px，剩下给编辑器不到 400px；更糟的是两个侧栏都写了
`collapsible={false}`，连收起来的余地都没有。

**修复**：写作页不再问 shell「这是不是手机」，改问「这一页摆不摆得下三栏」——
`min(screen.width, screen.height) >= 560`，取设备最短边。

取 `screen` 而不是 `innerHeight` 是有意的：平板上软键盘会把视口高度砍掉一半，
按视口高度判定会在**打字打到一半时把布局翻掉**。`screen` 不随键盘变化，且跟随转屏。
阈值 560 落在手机（约 400）和平板（约 800）之间。

同一次改动里，写作页里所有原先按 `isMobile` 分支的行为——单标签页模式、滑动手势、
新建章节后开单页、加选区后自动展开面板——都改成按这个新条件判断。**其中最容易漏的是
对话面板宿主的注册**：它当时仍按 `isMobile` 注册，横屏下根本不注册，于是 agent 按钮点了
没有任何反应（面板压根没挂载）。这类"布局换了但状态判断没跟着换"的地方，改的时候要一并过一遍。
